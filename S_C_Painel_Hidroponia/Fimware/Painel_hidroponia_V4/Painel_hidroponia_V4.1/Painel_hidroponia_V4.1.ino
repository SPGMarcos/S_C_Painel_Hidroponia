...
void publishMqttStatus() {
    if (mqttClient.connected()) {
        mqttClient.publish((mTopic + "/auto/state").c_str(), trava24h ? "ON" : "OFF", true);
        mqttClient.publish((mTopic + "/bomba/state").c_str(), v1 ? "ON" : "OFF", true);
        mqttClient.publish((mTopic + "/oxigenador/state").c_str(), v2 ? "ON" : "OFF", true);
    }
}

// Configuração de Auto-Discovery (Otimizada para não cortar o pacote)
void sendDiscovery() {
    String chipId = String((uint32_t)ESP.getEfuseMac(), HEX);
    
    struct Discovery { String sub; String name; String icon; };
    Discovery sensors[] = {
        {"auto", "Modo Automatico", "mdi:autorenew"},
        {"bomba", "Bomba Dagua", "mdi:water-pump"},
        {"oxigenador", "Oxigenador", "mdi:air-filter"}
    };

    for (auto &s : sensors) {
        String configTopic = "homeassistant/switch/hidro_" + chipId + "_" + s.sub + "/config";
        
        DynamicJsonDocument doc(600);
        doc["name"] = s.name;
        doc["stat_t"] = mTopic + "/" + s.sub + "/state";
        doc["cmd_t"] = mTopic + "/" + s.sub + "/set";
        doc["pl_on"] = "ON";
        doc["pl_off"] = "OFF";
        doc["icon"] = s.icon;
        doc["uniq_id"] = "hidro_" + chipId + "_" + s.sub;
        
        JsonObject dev = doc.createNestedObject("device");
        dev["ids"] = "hidro_" + chipId;
        dev["name"] = "Smart Hidroponia";
        dev["mf"] = "Marcos";
        dev["mdl"] = "Heltec V2";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    if (t == mTopic + "/auto/set") {
        trava24h = (msg == "ON");
        if(trava24h) { v1 = true; v2 = true; ultimoTempoBomba = millis(); }
    }
    else if (!trava24h) { // Só aceita comando manual se Automático estiver OFF
        if (t == mTopic + "/bomba/set") v1 = (msg == "ON");
        if (t == mTopic + "/oxigenador/set") v2 = (msg == "ON");
    }
...
void reconnectMQTT() {
    ...
    if (!mqttClient.connected()) {
        if (millis() - lastMqttRetry > 5000) {
            ...
            mqttClient.setBufferSize(1024); // Aumenta o buffer para aceitar o Discovery
            
            String clientId = "Hidro-ESP-" + String(random(0xffff), HEX);
            if (mqttClient.connect(clientId.c_str(), mUser.c_str(), mPass.c_str())) {
                mqttClient.subscribe((mTopic + "/auto/set").c_str());
                mqttClient.subscribe((mTopic + "/bomba/set").c_str());
                mqttClient.subscribe((mTopic + "/oxigenador/set").c_str());
                sendDiscovery();
                publishMqttStatus();
            }
        }
...
void handleSettings() {
    ...
    h += "<button onclick=\"if(confirm('Tem certeza que deseja apagar tudo?')){location.href='/factory_reset';}\" class='btn btn-red'>FACTORY RESET</button>";
